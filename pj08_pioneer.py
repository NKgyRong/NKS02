...
def draw_panel(ax, rec, period_label, name_map, bar_color):
    ...
    right = ax.inset_axes([0.31, 0.10, 0.685, 0.80])  # 0.31 + 0.685 ~ 0.995
    ...
    # Overlay axis for IPC code + subclass name (highest zorder so it is never covered)
    overlay = ax.inset_axes([0.0, 0.0, 1.0, 1.0], zorder=10)
    overlay.set_axis_off()
    product_line = PRODUCT_LINES.get(sub3, "")
    # IPC code + subclass name
    ax.text(
...
def main():
    ...
    # Prepare rows for plotting (sorted within each period)
    period_to_rows: Dict[str, List[dict]] = {p[0]: [] for p in PERIODS}
    for _, r in out_df.iterrows():
        period_to_rows[r["Period"]].append(r.to_dict())
    for p in period_to_rows:
        period_to_rows[p].sort(key=lambda d: (-(d.get("Period_total_amount") or 0.0), d.get("IPCsubclass", "")))

    targets_by_period = {p[0]: [sub for sub, per in TARGETS if per == p[0]] for p in PERIODS}
    for per in targets_by_period:
        wanted = set(targets_by_period[per])
        filt = [rec for rec in period_to_rows[per] if rec["IPCsubclass"] in wanted]
        while len(filt) < 5:
            filt.append({
                "IPCsubclass": "",
                "Period": per,
